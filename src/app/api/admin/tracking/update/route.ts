...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[TRACKING UPDATE] Iniciando actualización manual de tracking...')
    ...
    console.log(`[TRACKING UPDATE] Encontrados ${shipments.length} envíos para actualizar`)
    ...
    for (const shipment of shipments) {
      try {
        if (!shipment.glsReference) {
          console.log(`[TRACKING UPDATE] Envío sin referencia GLS, saltando`)
          continue
        }

        console.log(`[TRACKING UPDATE] Actualizando envío ${shipment.glsReference}`)

        const trackingInfo = await glsService.getTrackingByReference(shipment.glsReference)

        if (!trackingInfo) {
          console.log(`[TRACKING UPDATE] No se pudo obtener tracking para ${shipment.glsReference}`)
          results.push({
        ...
        if (newStatus === 'DELIVERED' && shipment.order && shipment.order.status !== 'DELIVERED') {
          ...
          await prisma.orderStatusHistory.create({
            data: {
              ...
              status: 'DELIVERED',
              notes: 'Pedido entregado según tracking de GLS'
            }
          })
          ...
          if (orderWithItems) {
            sendOrderStatusUpdateEmail(orderWithItems, 'DELIVERED').catch(err =>
              console.error('[TRACKING UPDATE] Error enviando email de entrega:', err)
            )
          }

          delivered++
          console.log(`[TRACKING UPDATE] ✅ Pedido ${shipment.order.orderNumber} marcado como ENTREGADO`)
        }
        ...
        console.error(`[TRACKING UPDATE] Error actualizando ${shipment.glsReference}:`, error)
        results.push({
          shipmentId: shipment.id,
          glsReference: shipment.glsReference,
          orderNumber: shipment.order?.orderNumber,
    ...
    console.log(`[TRACKING UPDATE] Completado: ${updated} actualizados, ${delivered} entregados`)
    ...
    console.error('[TRACKING UPDATE] Error general:', error)
    return NextResponse.json(
